...
/*   Updated: 2024/05/22 18:20:46 by abadouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	mlx_do_effects(void *param)
{
	...
	mlx_enemy_attack(data, data->pos, count);
...
static int	mlx_move_player(int key, void *param)
{
	...
	if (key == KEY_ESCAPE)
		exit_game(data, key);
	if (key == KEY_UP && !ft_strchr("1EN", data->map.up))
		mlx_move_up(data, key);
	else if (key == KEY_DOWN && !ft_strchr("1EN", data->map.down))
		mlx_move_down(data, key);
	else if (key == KEY_LEFT && !ft_strchr("1EN", data->map.left))
		mlx_move_left(data, key);
	else if (key == KEY_RIGHT && !ft_strchr("1EN", data->map.right))
...
void fn(){system("leaks so_long_bonus");}
int	main(int ac, char **av)
{
	...
	(mlx_checker(&data), mlx_game_render(&data), mlx_draw_string(&data));
